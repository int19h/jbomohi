...
The 2000 bracket-format log (`2000_05_26--2000_10_28.txt`) carries no day markers and is projected as one bounded unit `irc/lojban/2000/2000-05-26--2000-10-28.txt`, `Source-Id: 2000-05-26..2000-10-28`, `Time-Confidence: window`, with a `-- day boundary <n>` line at each observed clock rollover; no per-day files are fabricated. Its header is `# irc #lojban 2000-05-26..2000-10-28 tz=unknown source=<archive path> format=bracket days=unknown`; second-less times use seconds `00`.

**Events.** One commit per day file (`Event: import`, or `edited` when an archive day changes), author `irclogs <irclogs@irc.lojban.org>`, date = last message time of the day in its timezone, `Source: irc/<channel>`, `Source-Id: <date>`. The bounded 2000 unit is one `Event: import` with git date `2000-10-28T23:59:59+00:00`, `Source-Id: 2000-05-26..2000-10-28`, `Time-Confidence: window`, and `Event-Window: 2000-05-26..2000-10-28`. `_meta/irc/<channel>/days.csv` (`date,lines,messages,nicks,tz,format,source,days_observed,days_in_range`; the last two fields are empty for ordinary day files).
