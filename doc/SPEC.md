...
Status: **draft v0.5, 2026-08-27**. Authors: Fable (spec), the human partner (adjudication). Implementer: Codex. Changes are recorded in `doc/decisions/` (`2026-08-27-scope-and-policies.md`, `2026-08-27-repo-identities-sources.md`, `2026-08-27-dump-schemas.md`, `2026-08-27-mail-sources.md`); the deferred v0.1 material (indexes, a librarian service, Discord/web/MCP interfaces) is preserved in `doc/future/librarian-service.md` and is **out of scope**.
...
- **Excluded**: `llg-board`, `llg-members`, `special*` (HTTP 401 — restricted); `jbovlaste-admin` (≈80k automated notifications; excluded by default, listed in coverage as available).
...
1. **`jbovlaste-admin`** (≈80k automated dictionary-change notifications, 2000s–): excluded by default from `mail/`; include as a separate directory if the dictionary projection turns out to lack change dates it could supply (revisit after M2's dictionary import).
2. **Dump production** — the operator-side export commands in `doc/research/dump-schemas.md` §1.4, §3.9 and §4 (column-filtered `users` tables, no IP columns, Tiki charset flags) need to be agreed with whoever runs the export; the schemas themselves are settled (`2026-08-27-dump-schemas.md`, `2026-08-27-mail-sources.md`).
