## 2003-10-13T18:16:05Z — rlpowell (comment 111, on definition 14297)

Missing stuff.

Please use things like x_1=p_1, and I would think that at least one place
of skami should be in there (the computer, or type thereof, that the
program was written for).

-Robin


## 2003-10-13T19:57:59Z — arj (comment 112, on definition 14297, in reply to 111)

Re: Missing stuff.

rlpowell wrote:
> Please use things like x_1=p_1,

Then why didn't you say so on
http://www.lojban.org/jbovlaste/help/definitions.html, particularly before
I wrote hundreds of definitions without x_1 in them?

Grrr.

> and I would think that at least one place
> of skami should be in there (the computer, or type thereof, that the
> program was written for).

You may have a point there. However, I was merely sanitising the data
imported from Noralujv.


## 2003-10-13T20:11:08Z — rlpowell (comment 113, on definition 14297, in reply to 112)

Re: Missing stuff.

arj wrote:
> rlpowell wrote:
> > Please use things like x_1=p_1,
> 
> Then why didn't you say so on
> http://www.lojban.org/jbovlaste/help/definitions.html, particularly
before
> I wrote hundreds of definitions without x_1 in them?
> 
> Grrr.

Huh.

OK, I take it back.  Sorry.  Do it either way, as you like.

-Robin

